...
const smilesURL string = "https://www.smiles.com.ar"

type (
	// Load waits until the page is fully loaded
	Load func(ctx context.Context, relativeURL string, timeout time.Duration) error
	...
	OpenNewTab func(ctx context.Context, relativeURL string, timeout time.Duration) error
...
func MakeOpenNewTab(wd selenium.WebDriver, loadPage Load) OpenNewTab {
	return func(ctx context.Context, relativeURL string, timeout time.Duration) error {
		...
		err = loadPage(ctx, relativeURL, timeout)
		if err != nil {
			log.Error(ctx, err.Error())
